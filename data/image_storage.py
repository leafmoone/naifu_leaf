...
def load_parquet_lookup(parquet_path, source_type="dan", cache_dir=None):
    if not parquet_path or not os.path.exists(parquet_path):
        logger.error(f"Parquet path invalid: {parquet_path}")
        return {}

    parquet_path = Path(parquet_path)
    cache_filename = f"{parquet_path.stem}_{source_type}.lookup_cache.pkl"
    ...
    logger.info(f"No valid cache found. Processing {source_type} Parquet: {parquet_path}")
    start_t = time.time()
    
    if source_type == "e621":
        query_cols = "id::VARCHAR as id_str, width, height, general, artist, copyright, character, species, rating, year, nsfw, resolution, e621"
    else:
        query_cols = "id::VARCHAR as id_str, general, rating, artist, character, height, width, copyright, year, resolution, nsfw, aesthetics, character_core_tags, character_image_count, character_repeat"
        
    lookup = {}
    with duckdb.connect(database=':memory:') as con:
        p_sql = str(parquet_path).replace('\\','/')
        df = con.execute(f"SELECT {query_cols} FROM read_parquet('{p_sql}')").df()
    ...
    for _, row in df.iterrows():
        ...
        if source_type == "e621":
            tags = {
                ...
                "species": flatten_tags(row['species']),
                ...
                "e621_tag": flatten_tags(row['e621']),
                "rating": flatten_tags(row['rating']),
            }
            lookup[clean_id] = {
                "tags": tags,
                "source_type": "e621",
                "res": (safe_int(row['height']), safe_int(row['width'])),
                "character_core_tags": "",
            ...
            lookup[clean_id] = {
                "tags": {
                    ...
                    "resolution": flatten_tags(row['resolution']),
                    "nsfw": flatten_tags(row['nsfw']),
                    "aesthetics": flatten_tags(row.get('aesthetics', '')),
                },
                "source_type": "dan",
...
class ChunkedDirectoryImageStore(StoreBase):
    def __init__(self, *args, **kwargs):
        ...
        parquet_path_str = kwargs.get("parquet_path")
        self.parquet_path = Path(parquet_path_str)
        super().__init__(*args, **kwargs)
        self.dataset_source = kwargs.get("dataset_source", "dan")
        label_ext = self.kwargs.get("label_ext", ".txt")
        # self.parquet_path = kwargs.get("parquet_path", None)
        self.lookup_dict = load_parquet_lookup(self.parquet_path, source_type=self.dataset_source)if self.parquet_path else {}
        start_t = time.time()
    ...
    def process_single_entry(self, p, lookup=None):
        try:
            ...
            if match_id:
                ...
                stype = data.get("source_type", "dan")
                tags_dict = data["tags"]
